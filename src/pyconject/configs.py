...
class Configs:
    ...
    def _create_or_get_module_config(self, func):
        ...
        for mp in module_parts:
            if mp not in module_config.keys(): module_config[mp] = {}
        ...
        if _PYCONJECT not in module_config: 
            ...
            if module_config[_PYCONJECT][_PYCONJECT_INIT]:
                logger.warn(f"{func.__name__} in {func.__module__} is registered in pyconject after pyconject has already initialized configs for {func.__module__}.")
    ...
    def _resolve_module_configs(self, module_configs):
        ...
        default_module_config_path = {k: module_dir.joinpath(f"{module_name}-{k}-configs.yml") for k in ["dev", "stg", "prd"]}
        default_module_config_path[""] = module_dir.joinpath(f"{module_name}-configs.yml")
        ...
        if hasattr(module, _PYCONJECT_CONFIG_PATH):
            ...
            if isinstance(dev_module_config_path, dict):
                module_config_path = _merge_configs(dev_module_config_path, default_module_config_path)
        ...
        try:
            ...
            if self.override_by_target: logger.warn(f"Developer of module {module} does not specify configs for target {self.target}.")
        except FileNotFoundError as e:
            if self.override_by_target: logger.warn(f"Unable to find {eff_module_config_path} for {self.target} to configure module {module}.")

    def _resolve_user_configs(self, force=False):
        if self.resolve_user_configs:
            if force:
                logger.warn("User configs are being resolved after pyconject has already resolved user configs.")
            else:
                return 

        user_configs_filepaths = {k: f"./{k}-configs.yml" for k in ["dev", "stg", "prd"]}
        ...
        try:
            ...
            if self.override_by_filename:
                logger.error(f"Unable to find {eff_user_configs_filepath}. Please provide the configs file.")
                raise FileNotFoundError(e)
            self.user_configs = {}


    def get_configs(self, func, force=False):
        if not self.resolve_called: self.resolve()
        module_config = self._create_or_get_module_config(func)
...
def get_module_config_path(func, target=None):
    ...
    postfix = "-configs.yml"
    if target is not None: postfix = f"-{target}{postfix}"
